...
agents_UAV_pos = [[1.5, 0.5, 0.1], [1.5, -1.5, -0.2], [1.2, -0.8, 2]]
...
class Motivation(Slide):
    def construct(self):
        ...
        Title = Text("0. Motivation", font_size=28).to_corner(UL)

        news_1_en = Text(
            "Airports in Belgium and Sweden close due to drones",
            font_size=28
        )

        news_1_pt = Text(
            'Original: "Drones levam a encerramento de aeroportos na Bélgica e Suécia"',
            font_size=16
        )
        ...
        logo_publico = ImageMobject(
            "assets/images/logo_Publico.png"
        ).scale(IMAGE_SCALE)

        news_with_logo_1 = Group(
            logo_publico,
            news_1,
        ).arrange(
            RIGHT,
            buff=0.6
        ).shift(DOWN).scale(0.8)

        news_2_en = Text(
            "Bruxels Airport closes due to the presence of drones in its aerial space",
            font_size=28
        )
        news_2_pt = Text(
            'Original: "Aeroporto de Bruxelas encerra espaço aérep devido à presença de drones"',
            font_size=16
        ) 
        ...
        logo_RTP = ImageMobject(
            "assets/images/logo_RTP.png"
        ).scale(IMAGE_SCALE)

        news_with_logo_2 = Group(
            logo_RTP,
            news_2,
        ).arrange(
            RIGHT,
        ...
        # Animation must happen before next_slide()
        self.play(
            FadeIn(Title),
            FadeIn(news_with_logo_1),
            FadeIn(news_with_logo_2),
        )
...
class Intro(ThreeDSlide):
    def construct(self):
        Title = Text("1. Introduction", font_size=28).to_corner(UL)
...
class Objective(ThreeDSlide):    
    def construct(self):
        Title = Text("2. Objective", font_size=28).to_corner(UL)
        self.add_fixed_in_frame_mobjects(Title)
        # ============================================================
        ...
        self.next_slide()
        # --------------------------------------------------
        # Rotate q by pi while keeping r constant
        # --------------------------------------------------
        ...
        q_star = (
            MathTex(r"q^\star = [ 0.07 -0.66 -0.74]", color=WHITE, font_size=36)
        )
        equations_2 = VGroup(
            r_star,
            q_star 
        ...
        # --------------------------------------------------
        # Move to desired position p*
        # --------------------------------------------------

        # Stop the previous radial/rotation updater
        remaining_agent.clear_updaters()

        self.play(
            remaining_agent.animate.move_to(target_on_axis + p_stars_positions[2]),
            run_time=3,
        )
...
class FibonacciSphere(ThreeDSlide):
    ...
    def construct(self):
        Title = Text("3. Fibonacci Sphere", font_size=28).to_corner(UL)
        self.add_fixed_in_frame_mobjects(Title)
        # ============================================================
        # Helper: create the split-screen layout
        # ============================================================
        ...
        self.play(FadeIn(fib_time))
        self.next_slide()

class ControllerDesign(Slide):
    def construct(self):
        Title = Text("4. Controller Design", font_size=28).to_corner(UL)
        # ============================================================
        # Helper: create the split-screen layout
        # ============================================================
        ...
        def store_equation(eq, fadein=False, buff=0.5):
            nonlocal stored_y
            ...
            # Animate the copy moving to the stored position
            if fadein:
                ...
                self.play(
                    eq_copy.animate.move_to([stored_x, stored_y, 0]),
                    run_time=2
                )
        ...
        stored_proj_eq = store_equation(proj_eq, False)
        ...
        stored_dynamics = store_equation(dynamics, True)
        store_equation(geometric_err, True)
...
class APF(ThreeDSlide):
    def construct(self):
        ...
        for agent in agents_list:
            animation.append(
                agent.animate.shift(-target_uav_position)
            )

        self.play(
            *animation,
            sphere.animate.shift(-target_uav_position),
            equator.animate.shift(-target_uav_position),
            target_uav.animate.move_to(AXIS_ORIGIN)
        )
        ...
        # Get current vectors relative to sphere center
        ...
        # Points exactly R away from AXIS_ORIGIN
        q_i = AXIS_ORIGIN + R * v_i
        q_j = AXIS_ORIGIN + R * v_j

        self.play(
            FadeOut(target_uav),
            FadeOut(agents_list[2]),

            # Scale sphere from radius 1 -> radius 2.5
            sphere.animate.scale(R),
            equator.animate.scale(R),

            # Move BOTH UAVs to radius 2.5
        ...
        # Projected screen positions for fixed labels
        label_i.move_to(self.camera.project_point(q_i) + UP * 0.35)
        label_j.move_to(self.camera.project_point(q_j) + UP * 0.35)

        self.add_fixed_in_frame_mobjects(label_i, label_j)
        self.play(
...
class Radial_control(Slide):
    def construct(self):
        Title = Text("6. Radial Control", font_size=28).to_corner(UL)
        # ============================================================
        # Helper: create the split-screen layout
        # ============================================================
        ...
        dot_psi = MathTex(
            r"\dot \Psi = -k_{geo}\|e_{q_i}\|^2",
            font_size=34
        )

        # Store the equations already established
        stored_dynamics = store_equation(dynamics, fadein=True)
        stored_r_dot = store_equation(r_dot, fadein=True)
        stored_psi_dot = store_equation(dot_psi, fadein=True)
        ...
        # ============================================================
        stored_dot_V_eq = store_equation(dot_V_eq)
        self.play(
            FadeOut(dot_V_eq),
            FadeOut(V_eq),
        )
        ...
        dot_V_eq = VGroup(lhs, dot_V_equal, rhs)
        dot_V_eq.next_to(active_title, DOWN, buff = 0.18)
        self.play(
            FadeOut(dot_er_eq),
            FadeIn(dot_V_eq)
        )
        ...
        dot_V_results = MathTex(
            r"\left( k_{geo}>0, \quad k_{rad}>0, \quad e^{-\|e_{q_i}\|^2}>0  \right) \forall e_{q_i} \Longrightarrow \dot V \le 0",
            font_size=30
...
class Conclusions(Slide):
    def construct(self):
        Title = Text("7. Conclusions", font_size=28).to_corner(UL)

        conclusion_text = Text(
            "Introduced: \n" \
            "- Desired point encirclement\n" \
            "- Scalar and vectorial goal separation\n"\
            "- Proof of almost global convergence",
            font_size=28
        ).shift(LEFT*3.5)

        IMAGE_SCALE = 0.6
        qr_title = Text("Code available at github:", font_size=32).shift(RIGHT*3+UP*3)

        qr_code_github = ImageMobject(
            "assets/images/qr_code_github.png"
        ).scale(IMAGE_SCALE).shift(RIGHT*3)

        self.play(
            FadeIn(Title),
            FadeIn(conclusion_text),
            FadeIn(qr_title),
            FadeIn(qr_code_github)
        )
...
class test(Slide):
    def construct(self):
        lhs = MathTex(r"\dot q_i", font_size=40)
        sphere_equal = MathTex("=", font_size=40)
        rhs = MathTex("S(\omega_i)", "q_i", font_size=40)

        sphere_dynamic = VGroup(lhs, sphere_equal, rhs)
        sphere_dynamic.arrange(RIGHT, buff=0.15)
        ...
        # Make a copy of the ORIGINAL equation and move it below
        sphere_dynamic_new = sphere_dynamic.copy()
        self.add(sphere_dynamic_new)

        self.play(
            sphere_dynamic_new.animate.next_to(sphere_dynamic, DOWN, buff = 0.35),
            run_time=1.5
        )

        # Get the copied lhs
        new_lhs = MathTex(
            r"-k_{geo}S(e_{q_i})", "q_i",
            font_size=40
        ).next_to(
            sphere_dynamic_new[1], LEFT,
            buff=0.15
        )

        # Transform ONLY the lower copy
        self.play(
            TransformMatchingTex(
                sphere_dynamic_new[0],
                new_lhs,
                run_time=3
            )
        )

        # Optional: update reference to lower lhs
        lhs = new_lhs
